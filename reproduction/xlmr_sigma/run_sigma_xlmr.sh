...
FABLE_RUN_DIR="${1:?usage: run_sigma_xlmr.sh <full-fable-run-dir> [layers] [sequence-index]}"
LAYERS="${2:-1}"
SEQUENCE_INDEX="${3:-0}"
...
OUT_DIR="$FABLE_RUN_DIR/sigma-${LAYERS}layer-seq${SEQUENCE_INDEX}"
KEY_PREFIX="$OUT_DIR/keys/xlmr${LAYERS}-seq${SEQUENCE_INDEX}"
LIBS="/home/slf/anaconda3/envs/sigma-fable/lib:/usr/local/cuda-12.1/lib64:$ROOT_DIR/third_party/EzPC/GPU-MPC/ext/sytorch/build:$ROOT_DIR/third_party/EzPC/GPU-MPC/ext/sytorch/build/ext/cryptoTools:$ROOT_DIR/third_party/EzPC/GPU-MPC/ext/sytorch/build/ext/llama:$ROOT_DIR/third_party/EzPC/GPU-MPC/ext/sytorch/build/ext/bitpack"

[[ -x "$BIN" ]] || { printf 'Run %s first.\n' "$SCRIPT_DIR/build_sigma_xlmr.sh" >&2; exit 1; }
[[ -f "$FABLE_RUN_DIR/verification.json" ]] || { printf 'FABLE run has not been verified.\n' >&2; exit 1; }
if [[ "$(python -c 'import json,sys; print(json.load(open(sys.argv[1]))["dimensions"][1])' "$FABLE_RUN_DIR/verification.json")" != 768 ]]; then
    printf 'Refusing partial FABLE input: a SIGMA recorded run requires all 768 dimensions.\n' >&2
    exit 1
fi
if [[ ! -f "$FABLE_RUN_DIR/sigma-input/verification.json" ]]; then
    "$SCRIPT_DIR/run_sigma_input_bridge.sh" "$FABLE_RUN_DIR"
fi
...
dealer_input="$FABLE_RUN_DIR/sigma-input/dealer/dealer-input-mask.u64"
...
(
    CUDA_VISIBLE_DEVICES="$P0_GPU" "$BIN" online "$LAYERS" 0 \
        "$SEQUENCE_INDEX" "$FABLE_RUN_DIR/sigma-input/P0-masked-input.u64" \
...
CUDA_VISIBLE_DEVICES="$P1_GPU" "$BIN" online "$LAYERS" 1 \
    "$SEQUENCE_INDEX" "$FABLE_RUN_DIR/sigma-input/P1-masked-input.u64" \
    "$masked_weights" "$KEY_PREFIX" 127.0.0.1 "$THREADS" \
...
{
  "status": "pass",
  "scheme": "FABLE embedding -> arithmetic shares -> SIGMA masked input -> XLM-R encoder",
  "layers": $LAYERS,
  "sequence_index": $SEQUENCE_INDEX,
  "fable_dimensions": 768,
  "p0_gpu": $P0_GPU,
  "p1_gpu": $P1_GPU,
